...
1) How significant is this relationship?
2) Can we parameterize the dependence of `y` on `DOSE` with some GLM?
...
Thus, our model will take the form `y ~ DOSE`, will be linear with categorical predictor for continuous outcome (ANOVA), and we are fine with the `lm` method. 
...
y_DOSE_reg = 
  ...
  parsnip::set_mode("regression") #mode = regression
...
y_DOSE_reg_WF = 
  ...
  workflows::add_recipe(rec_y_DOSE) #adding the recipe - which variables for outcome, precitor, any interactions...
